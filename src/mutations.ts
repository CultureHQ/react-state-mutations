...
export const makeStandaloneMutation = <T extends any>(apply: Apply<T>) => (
  (field: Field): StandaloneMutation => {
    ...
    mutation.standalone = true as const;
    return mutation;
  }
);

export const makeStandaloneHook = (
  <T extends any>(apply: Apply<T>, defaultValue: T) => (
    (initialValue = defaultValue): [T, () => void] => {
...
export const makeArgumentMutation = <T extends any, A extends any>(apply: ArgumentApply<T, A>) => (
  (field: Field): ArgumentMutation<A> => {
    ...
    mutation.standalone = false as const;
    return mutation;
  }
);

export const makeArgumentHook = (
  <T extends any, A extends (arg: any) => any>(apply: ArgumentApply<T, A>, defaultValue: T) => (
    (initialValue = defaultValue): [T, (argument: A) => void] => {
      const [value, setValue] = useState<T>(initialValue);
...
export const appendState = <T extends any>(argument: T) => (value: T[]): T[] => (
...
export const concatState = <T extends any>(argument: T | T[]) => (value: T[]): T[] => (
  value.concat(argument)
);
...
export const cycleState = <T extends any>(argument: T[]) => (value: T): T => (
  argument[(argument.indexOf(value) + 1) % argument.length]
);

export const cycle = makeArgumentMutation(cycleState);
export const useCycle = <T extends any>(argument: T[]): [T, () => void] => {
  const [value, setValue] = useState<T>(argument[0]);

  return [value, (): void => setValue(cycleState<T>(argument))];
};

export const directState = <T extends any>(argument: T) => (): T => argument;
export const direct = makeArgumentMutation(directState);

type FilterArgument<T> = ((value: T) => boolean);

export const filterState = <T extends any>(argument: FilterArgument<T>) => (value: T[]): T[] => (
...
export const mapState = <T extends any>(argument: MapArgument<T>) => (value: T[]): T[] => (
  value.map(argument)
);
...
export const mutateState = (argument: State) => (
  (value: State): State => ({ ...value, ...argument })
);
export const mutate = makeArgumentMutation(mutateState);

export const prependState = <T extends any>(argument: T) => (value: T[]): T[] => (
...
const normalizeMutation = (mutation: (CombineMutation | State)): CombineMutation => {
  ...
  directMutation.standalone = true as const;
...
export const combineMutations = (...mutations: (CombineMutation | State)[]): Apply<State> => {
  ...
  if (normalized.some(mutation => !mutation.standalone)) {
    return (...args: any[]) => (state: State): State => {
      let argIndex = -1;
  ...
  return (state: State): State => normalized.reduce(
